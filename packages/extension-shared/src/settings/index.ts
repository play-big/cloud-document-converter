...
import { EventName } from '../message'
import type { Port } from '@dolphin/common/message'
...
export const createGetSettings = (options: GetSettingsOptions) => {
  const { port } = options

  return async <Key extends keyof Settings>(
    keys: Key[],
